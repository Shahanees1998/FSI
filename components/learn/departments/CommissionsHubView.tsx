...
import { COMMISSIONS_NAV } from "@/lib/learn/departmentsCommissionsNav";

const link = "text-blue-600 font-medium no-underline hover:underline";

export default function CommissionsHubView() {
    return (
            ...
            <div className="p-4 md:p-5 lg:p-6" style={{ maxWidth: "48rem" }}>
                ...
                <ul className="m-0 p-0 list-none border-1 surface-border border-round-lg overflow-hidden">
                    {COMMISSIONS_NAV.map(({ slug, label }, i) => (
                        <li
                            key={slug}
                            className={`surface-ground ${i < COMMISSIONS_NAV.length - 1 ? "border-bottom-1 surface-border" : ""}`}
                        >
                            <Link
                                href={`/agent/learn/departments/commissions/${slug}`}
